...
- (NSString*)convertFromHours:(NSString*)hours minutes:(NSString*)minutes {
    ...
    NSArray *wordsNums = @[@"zero", @"one", @"two", @"three", @"four",@"five",
    ...
    if (m == 0) {
        ...
        return [NSString stringWithFormat:@"%@ minutes past %@", wordsNums[m],
                wordsNums[h]];
    } else if (m == 45) {
        return [NSString stringWithFormat:@"quarter to %@", wordsNums[h + 1]];
    } else if (m > 30) {
        int m1 = 60 - m;
        return [NSString stringWithFormat:@"%@ minutes to %@", wordsNums[m1],
                wordsNums[h + 1]];
    }
    return @"";
}
